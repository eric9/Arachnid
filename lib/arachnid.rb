		...
		while(@global_queue.size > 0 && (max_urls.nil? || @global_visited.size.to_i < max_urls))
			...
			temp_queue.each do |q|
				...
				begin
					...
					request = Typhoeus::Request.new(q, :user_agent=>'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', :timeout => 10000, :follow_location => true) if ip == nil
					request = Typhoeus::Request.new(q, :user_agent=>'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', :timeout => 10000, :follow_location => true, :proxy => "#{ip}:#{port}") if ip != nil && user == nil
					request = Typhoeus::Request.new(q, :user_agent=>'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', :timeout => 10000, :follow_location => true, :proxy => "#{ip}:#{port}", :proxy_username => user, :proxy_password => pass) if user != nil
